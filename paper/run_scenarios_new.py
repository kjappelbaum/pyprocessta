...
model_cov3 = TCNModelDropout()

model_cov4 = TCNModelDropout()


FEAT_NUM_MAPPING = dict(zip(MEAS_COLUMNS, [str(i) for i in range(len(MEAS_COLUMNS))]))
UPDATE_MAPPING = {
    "amp": {
        "scaler": joblib.load("20220116_2-Amino-2-methylpropanol_y_scaler"),
        "model": model_cov1.load_model(
            os.path.join(
                THIS_DIR, "20220116_2-Amino-2-methylpropanol_model_darts.pth.tar"
            )
        ),
        "name": ["2-Amino-2-methylpropanol C4H11NO"],
    },
    "pz": {
        "scaler": joblib.load("20220116_Piperazine C4H10N2_y_scaler"),
        "model": model_cov1.load_model(
            os.path.join(THIS_DIR, "20220116_Piperazine_model_darts.pth.tar")
        ),
        "name": ["Piperazine C4H10N2"],
    },
    "co2": {
        "scaler": joblib.load("20220116_Carbon dioxide CO2_y_scaler"),
        "model": model_cov2.load_model(
            os.path.join(THIS_DIR, "20220116_Carbon_model_darts.pth.tar")
        ),
        "name": ["Carbon dioxide CO2"],
    },
    "nh3": {
        "scaler": joblib.load("20220116_Ammonia NH3_y_scaler"),
        "model": model_cov2.load_model(
            os.path.join(THIS_DIR, "20220116_Ammonia_model_darts.pth.tar")
        ),
        "name": ["Ammonia NH3"],
    },
}
SCALER = joblib.load("20211229_Piperazine C4H10N2_x_scaler")
